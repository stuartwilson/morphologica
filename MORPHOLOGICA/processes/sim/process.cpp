...
int main(int argc, char **argv) {
    ...
    displays.push_back(Gdisplay(600,"pup view",4.,0.,0.));
    ...
    double mint2 = 0.;
    double maxt2 = 0.25;
    double oneOverMaxtMinusMint2 = 1./(maxt2-mint2);
    ...
    while(doing){
        ...
        out<<macrohuddle<<","<<microhuddle<<","<<correl<<",";
        ...
        switch(stoi(command[0])){
            ...
            case 1:{//W.logfile<<W.processName<<"@"<<TIMEcs<<": 1=STEP"<<endl<<flush;
                ...
                for(int i=0;i<A[F].nSOM;i++){
                    double x = A[F].win[i][0]-A[F].win[i][1];
                    double y = A[F].wout[i][0];
                    sumXY += x*y;
                    sumX += x;
                    sumY += y;
                    sumXX += x*x;
                }
                double SP = sumXY-(sumX*sumY)/(double)A[F].nSOM;
                double SSx = sumXX-sumX*sumX/(double)A[F].nSOM;
                correl = SP/SSx;
            ...
            case 8:{//W.logfile<<W.processName<<"@"<<TIMEcs<<": 8=DISA"<<endl<<flush;
                ...
                for(int I=0;I<A.size();I++){
                    ...
                    for(int i=0;i<A[I].nP;i++){
                        if(A[I].contacts[i]==1){
                            cl.push_back(getJetColor(fmin(fmax((A[I].Tc[i]-Ta-mint2)*oneOverMaxtMinusMint2,0.),1.)));
                        } else {
                            cl.push_back(getJetColor(fmin(fmax((A[I].Tb-Ta-mint2)*oneOverMaxtMinusMint2,0.),1.)));
                        }
                    }
                    displays[0].drawSphereFromMesh(Xsp,A[I].sphereM,cl);
                }
                // indicate focal pup
                displays[0].drawCylinder(A[F].Xnow[0](0),A[F].Xnow[0](1),A[F].Xnow[0](2),A[F].Xnow[0](0),A[F].Xnow[0](1),A[F].Xnow[0](2)+0.6,1e-6,0.02,12,vector<double>(3,0.));//vector<double>(3,0.95));
                
                displays[0].drawCylinder(A[F].Xnow[0](0),A[F].Xnow[0](1),A[F].Xnow[0](2)+0.6,boundX,0.1,floorH,0.0025,0.0025,12,vector<double>(3,0.));//vector<double>(3,0.95));
                displays[0].drawCylinder(A[F].Xnow[0](0),A[F].Xnow[0](1),A[F].Xnow[0](2)+0.6,boundX,-0.1,floorH,0.0025,0.0025,12,vector<double>(3,0.));//vector<double>(3,0.95));
            ...
            case 9:{//W.logfile<<W.processName<<"@"<<TIMEcs<<": 9=DISB"<<endl<<flush;
                ...
                for(int i=0;i<A[F].H[0].size();i++){
                    ...
                    displays[1].drawHex(A[F].H[0][i]+0,A[F].H[1][i]+1*space,0.0,0.016,c[0],c[1],c[2]);
                }
                ...
                */
                ...
                for(int i=0;i<A[F].nP;i++){
                    //cl1.push_back(getJetColor((double)A[F].sphereC[i][0]/6.));
                            //cl1.push_back(getJetColor((double)A[F].sphereC[i][1]/(double)A[F].sphereN));
                            //cl1.push_back(getJetColor((double)A[F].sphereC[i][2]/(double)A[F].sphereN));
                            //cl1.push_back(HSVtoRGB(A[F].sphereS[i][0]/(2.*M_PI),1.,1.));
                            //cl1.push_back(HSVtoRGB(A[F].sphereS[i][1]/(M_PI),1.,1.));
                            //double mag = fmin(fmax((A[F].input[i]-mint)*oneOverMaxtMinusMint,0.),1.);
                            //cl1.push_back(vector<double> (3,mag));
                            cl1.push_back(getJetColor(fmin(fmax((A[F].Tc[i]-Ta-mint2)*oneOverMaxtMinusMint2,0.),1.)));
                        }
                        
                        
                displays[1].drawFlatCube(A[F].sphereC,cl1,0,-1*space,0);
